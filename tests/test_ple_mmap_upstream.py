...
import vllm.models.qwen3_8_flash_next.nvidia.model as model_module
import vllm.models.qwen3_8_flash_next.nvidia.model_state as model_state_module
import vllm.models.qwen3_8_flash_next.nvidia.ple_mmap as ple_mmap
...
from vllm.models.qwen3_8_flash_next.common.ple import copy_ple_embedding_shard_
from vllm.model_executor.layers.vocab_parallel_embedding import VocabParallelEmbedding as PLEVocabParallelEmbedding
from vllm.models.qwen3_8_flash_next.nvidia import ple_layer as ple_layer_module
...
# Compatibility shim: the pinned base has a PleOffloadLayer NGram constructor
# with explicit workspace sizes and a hidden_states argument (unused locally).
# No lookup/hash/staging algorithm is replaced by this shim.
_BaseNGramEmbedding = Qwen3_8FlashNextNGramEmbedding
class Qwen3_8FlashNextNGramEmbedding(_BaseNGramEmbedding):
    def __init__(self, config, embedding_dim, ple_dense_layer_id, prefix,
                 layer_name, quant_config=None, params_dtype=None):
        super().__init__(config, embedding_dim, ple_dense_layer_id,
                         2048, 256, prefix, quant_config, params_dtype)
        self.layer_name = layer_name

    def forward(self, input_ids, query_start_loc, ngram_context):
        return super().forward(input_ids, input_ids, query_start_loc, ngram_context)
...
# --------------------------------------------------------------------------- #
# (d) shard-mapping contract: quotes Qwen3_8FlashNextNGramEmbedding.load_weights's
...
def _upstream_expected_rows(
    ...
    # Verbatim from Qwen3_8FlashNextNGramEmbedding.load_weights, including the
...
def test_bound_exceeded_warns_exactly_once_per_table_across_varying_run_counts(
    ...
    try:
        with caplog_vllm.at_level(
            logging.WARNING, logger="vllm.models.qwen3_8_flash_next.nvidia.ple_mmap"
        ):
            for ids in (
...
def test_mmap_gather_op_is_not_registered() -> None:
    ...
    assert not hasattr(torch.ops.vllm, "qwen3_8_flash_next_ple_mmap_forward")
...
def test_mmap_gather_op_is_absent_from_v1_splitting_ops() -> None:
    ...
    assert "vllm::qwen3_8_flash_next_ple_mmap_forward" not in cc.splitting_ops
    # The pinned base's OTHER PLE custom op (short convolution)
    ...
    assert "vllm::qwen3_8_flash_next_ple_short_conv" in cc.splitting_ops
...
# above, but its CALL from Qwen3_8FlashNextNGramEmbedding.__init__ (ple_layer.py)
...
def test_ngram_embedding_forward_compiles_across_two_different_token_counts(
    ...
    with set_current_vllm_config(vllm_config):
        layer = Qwen3_8FlashNextNGramEmbedding(
            config,
            8,
            0,
...
class _PleDownstreamConsumer(nn.Module):
    ...
    `Qwen3_8FlashNextNGramEmbedding.forward` returns on its own in mmap mode -- which
    ...
    def __init__(
        self,
        ple_layer: Qwen3_8FlashNextNGramEmbedding,
        weight: torch.Tensor,
        out: torch.Tensor,
    ) -> None:
        super().__init__()
...
def _build_v2_model_state_for_ple(
    ...
    """A real `Qwen3_8FlashNextModelState` wired to `layer`, bypassing `__init__`'s
...
def test_v2_staging_survives_torch_compile_and_full_cudagraph_capture_replay(
    ...
    kernel (a GEMM) capturing and consuming `Qwen3_8FlashNextNGramEmbedding`'s
    output, and rows staged the same way the real runner stages them:
    through `Qwen3_8FlashNextModelState.prepare_inputs` / `prepare_dummy_inputs`,
    never a raw `gather_into` call from the test.

    `Qwen3_8FlashNextNGramEmbedding.forward` in mmap mode returns a bare view of
    ...
    with (
        ...
        def _stage_actual_request(
            ...
            `Qwen3_8FlashNextModelState.prepare_inputs`, and return the
...
def test_v2_staging_cudagraph_replay_red_proof_disconnected_staging(
    ...
    (`Qwen3_8FlashNextNGramEmbedding.prepare_mmap_rows` neutralized to a no-op, so
    ...
    with (
        ...
        patch.object(Qwen3_8FlashNextNGramEmbedding, "prepare_mmap_rows", lambda *a, **k: None),
    ):
        manager, desc = _capture_ple_consumer_fullgraph(
            monkeypatch,
            vllm_config,
...
def _build_mmap_ngram_layer(
    ...
    *,
    ...
    """A real V2-mode Qwen3_8FlashNextNGramEmbedding with mmap enabled, with no
    ...
    with set_current_vllm_config(vllm_config):
        return Qwen3_8FlashNextNGramEmbedding(
            config,
            embedding_dim,
            layer_idx,
...
class _RecordingMmapModule:
    """Stands in for a Qwen3_8FlashNextNGramEmbedding at the model-state boundary:
...
def test_build_tables_is_idempotent_and_reuses_an_already_attached_table(
    ...
    """Qwen3_8FlashNextForCausalLM.load_weights and
    Qwen3_8FlashNextForConditionalGeneration.load_weights both call build_tables on
...
# SAME Qwen3_8FlashNextNGramEmbedding.compute_ngram_ids as the stock arm and
...
def test_env_on_off_forward_equivalence_fp8_and_dequantized(
    ...
    # REGISTERED qwen3_8_flash_next_compute_ple_ngram_ids op (upstream architecture,
    ...
    monkeypatch.setattr(
        torch.ops.vllm,
        "qwen3_8_flash_next_compute_ple_ngram_ids",
    ...
    with set_current_vllm_config(vllm_config):
        mmap_module = Qwen3_8FlashNextNGramEmbedding(
            config,
            embedding_dim,
            0,
            "model.layers.1.ple.ple_embedding",
    ...
    stock_ple_layer = Qwen3_8FlashNextPLELayer.__new__(Qwen3_8FlashNextPLELayer)
    ...
    mmap_ple_layer = Qwen3_8FlashNextPLELayer.__new__(Qwen3_8FlashNextPLELayer)
...
# exact output of a fixed, small, real Qwen3_8FlashNextNGramEmbedding on fixed
...
# --------------------------------------------------------------------------- #
# Qwen3_8FlashNextNGramEmbedding's mmap staging buffer allocates
...
def test_mmap_forward_allocates_an_fp8_output_buffer(
    ...
    Qwen3_8FlashNextPLELayer._dequantize_embeddings would silently skip
...
def test_ple_layer_registers_its_own_prefix_as_the_static_forward_context_key(
    ...
    """Qwen3_8FlashNextPLELayer.__init__ passes its OWN prefix as
    layer_name — the exact key it registers into
    compilation_config.static_forward_context — never
    f"{prefix}.ple_embedding". Constructs a REAL Qwen3_8FlashNextPLELayer (not just
    ...
    with set_current_vllm_config(vllm_config):
        layer = Qwen3_8FlashNextPLELayer(
            config,
            vllm_config=vllm_config,
            layer_idx=1,
...
# --------------------------------------------------------------------------- #
# model.py load_weights hook: both Qwen3_8FlashNextForCausalLM and
# Qwen3_8FlashNextForConditionalGeneration must call build_tables when enabled —
...
@pytest.mark.parametrize(
    "cls_name", ["Qwen3_8FlashNextForCausalLM", "Qwen3_8FlashNextForConditionalGeneration"]
)
...
def test_model_load_weights_never_calls_build_tables_when_disabled(
    ...
    stub_self = SimpleNamespace(
        hf_to_vllm_mapper=model_module.Qwen3_8FlashNextForCausalLM.hf_to_vllm_mapper,
        model_config=_model_config(tmp_path),
    )
    cc = SimpleNamespace(static_forward_context={})
    vllm_config = SimpleNamespace(compilation_config=cc)
    ...
    with set_current_vllm_config(vllm_config):
        result = model_module.Qwen3_8FlashNextForCausalLM.load_weights(stub_self, iter([]))
...
# --------------------------------------------------------------------------- #
# The child-level guard in Qwen3_8FlashNextNGramEmbedding.load_weights
...
class _ReloadApprovalModel:
    ...
    `clear_reload_approval`, exactly like `Qwen3_8FlashNextForCausalLM` /
    `Qwen3_8FlashNextForConditionalGeneration` -- without building a real nn.Module
...
# --------------------------------------------------------------------------- #
# Nested `Qwen3_8FlashNextForConditionalGeneration.load_weights` transactions:
# validates against its own approval, then `AutoWeightsLoader`'s recursive
# walk of its module tree invokes the nested `Qwen3_8FlashNextForCausalLM`'s OWN
...
class _NestedReloadApprovalInnerModel:
    """Faithful stand-in for the nested `Qwen3_8FlashNextForCausalLM`, wired to the
...
class _NestedReloadApprovalOuterModel:
    """Faithful stand-in for `Qwen3_8FlashNextForConditionalGeneration` composing a
...
def test_causal_lm_preflight_grants_no_inner_model_attr(
    ...
    """`Qwen3_8FlashNextForCausalLM` (standalone, no multimodal wrapper) must pass
    ...
    with set_current_vllm_config(vllm_config):
        model_module.Qwen3_8FlashNextForCausalLM.preflight_reload_weights(
...
def test_conditional_generation_preflight_grants_inner_model_attr_language_model(
    ...
    """`Qwen3_8FlashNextForConditionalGeneration` must pass
    ...
    with set_current_vllm_config(vllm_config):
        model_module.Qwen3_8FlashNextForConditionalGeneration.preflight_reload_weights(
            stub_self, weights_path="/p", is_checkpoint_format=True
        )
...
def test_nested_load_weights_outer_and_inner_each_validate_their_own_approval(
    ...
    `Qwen3_8FlashNextForConditionalGeneration` classmethod); neither `load_weights`
    ...
    with set_current_vllm_config(vllm_config):
        ...
        model_module.Qwen3_8FlashNextForConditionalGeneration.clear_reload_approval(outer)
...
def test_nested_reload_clears_inner_approval_if_outer_raises_first(
    ...
    `Qwen3_8FlashNextForConditionalGeneration` classmethod) must still remove that
...
class _RealInterleavedInnerCausalLM(nn.Module):
    """Real `nn.Module` standing in for the nested `Qwen3_8FlashNextForCausalLM`:
...
class _RealInterleavedOuterConditionalGeneration(nn.Module):
    """Real `nn.Module` standing in for `Qwen3_8FlashNextForConditionalGeneration`:
...
class _InitialLoadInnerCausalLM(nn.Module):
    """Real nn.Module standing in for the nested `Qwen3_8FlashNextForCausalLM` on a
    ...
    `Qwen3_8FlashNextForCausalLM.load_weights`."""
...
class _InitialLoadOuterConditionalGeneration(nn.Module):
    """Real nn.Module standing in for `Qwen3_8FlashNextForConditionalGeneration` on
    ...
    returns -- exactly like the real `Qwen3_8FlashNextForConditionalGeneration.
    load_weights`."""
...
def test_standalone_causal_lm_initial_load_owns_and_clears_its_own_token() -> None:
    """A standalone `Qwen3_8FlashNextForCausalLM` initial load (no multimodal
...
# `model_module.Qwen3_8FlashNextForCausalLM.load_weights` /
# `model_module.Qwen3_8FlashNextForConditionalGeneration.load_weights` (bound onto
...
# `Qwen3_8FlashNextNGramEmbedding.load_weights` (nvidia/ple_layer.py) raises
...
# like the production `Qwen3_8FlashNextPLELayer` does.
# --------------------------------------------------------------------------- #


class _RealColdLoadPLEReceiver(nn.Module):
    """Real nn.Module reproducing the ONE guard in
    `Qwen3_8FlashNextNGramEmbedding.load_weights` this red-proof depends on: reject
...
class _RealColdLoadInnerCausalLM(nn.Module):
    """Real nn.Module standing in for `Qwen3_8FlashNextForCausalLM`: `load_weights`
    IS the real `model_module.Qwen3_8FlashNextForCausalLM.load_weights` (bound as a
    ...
    load_weights = model_module.Qwen3_8FlashNextForCausalLM.load_weights
    ...
    def __init__(
        ...
        super().__init__()
        self.hf_to_vllm_mapper = model_module.Qwen3_8FlashNextForCausalLM.hf_to_vllm_mapper
...
class _RealColdLoadOuterConditionalGeneration(nn.Module):
    ...
    `model_module.Qwen3_8FlashNextForConditionalGeneration.load_weights`."""

    load_weights = model_module.Qwen3_8FlashNextForConditionalGeneration.load_weights
    ...
    def __init__(
        ...
        self.hf_to_vllm_mapper = (
            model_module.Qwen3_8FlashNextForConditionalGeneration.hf_to_vllm_mapper
        )
        self.model_config = model_config
...
def _real_cold_load_setup(
    ...
    the production `Qwen3_8FlashNextPLELayer` composition."""
...
def test_real_production_cold_load_survives_interleaved_ple_shard_group(
    ...
    fail the same way: the first `Qwen3_8FlashNextForCausalLM.load_weights` call
    attaches a table from headers alone, and the interleaved second call --
    carrying the real PLE shard + scale -- reaches
    `Qwen3_8FlashNextNGramEmbedding.load_weights`'s real guard and raises "already
...
def test_real_production_cold_load_partially_fails_without_role_based_deferral(
    ...
    hits `Qwen3_8FlashNextNGramEmbedding.load_weights`'s real guard and raises --
...
def test_default_off_forward_never_calls_the_mmap_gather_op(
    ...
    # This base hashes directly in forward_impl; no ID custom-op shim is needed.
    input_ids = torch.tensor([1, 2], dtype=torch.long)
    query_start_loc = torch.tensor([0, 2], dtype=torch.long)
    ngram_context = torch.zeros((1, 4), dtype=torch.long)
...
def test_ple_layer_constructs_ple_vocab_embedding_once_in_env_off_arm() -> None:
    """The pinned base constructs VocabParallelEmbedding exactly once, in
    the resident arm; its custom shard-copy loader remains in use."""
    source = inspect.getsource(ple_layer_module)
    assert source.count("VocabParallelEmbedding(") == 1
